...
export async function extractLabs({ patient_id, storage_bucket, storage_path }) {
  ...
  // healthspan_biomarkers.value and .sampled_on are both NOT NULL. A marker
  // with no numeric value (the prompt returns value:null for qualitative
  // results) or a report with no parseable date cannot be stored as a
  // numeric trend row, so those rows are skipped rather than failing the
  // whole batch insert on a NOT NULL violation.
  const rows = [];
  let skipped = 0;
  for (const m of markers) {
    ...
    if (value === null || sampledOn === null) {
      skipped++;
      continue; // not storable as a numeric, dated biomarker row
    }
    rows.push({
      patient_id,
  ...
  if (rows.length === 0) {
    return { sampled_on: sampledOn, markers: [], inserted: 0, skipped };
  }
...
// JSON field -> frontdesk_intakes column. Only fields with a real column in
// public.frontdesk_intakes are written. document_number and expiry have no
// column in that table — they are still returned in the extracted response
// but are never used for an UPDATE.
const CARD_FIELD_TO_COLUMN = {
  full_name: "full_name",
  date_of_birth: "date_of_birth",
  insurance_provider: "insurance_provider",
  member_id: "insurance_member_id",
  group_number: "insurance_group_no",
};

// Fields the model returns that have no column in frontdesk_intakes.
const CARD_EXTRA_FIELDS = ["document_number", "expiry"];
...
export async function extractCard({ intake_id, storage_bucket, storage_path }) {
  ...
  // Normalise the model output into the documented field set. This includes
  // fields with no frontdesk_intakes column (document_number, expiry) so the
  // caller still sees them — they just are not written to the table.
  const extracted = {};
  for (const field of [...Object.keys(CARD_FIELD_TO_COLUMN), ...CARD_EXTRA_FIELDS]) {
